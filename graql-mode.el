...
(setq graql-font-lock-keywords
      (let* ()
            ...
            (x-keywords '("define" "undefine" "match" "get" "insert" "delete" "compute"))
            (x-types '("long" "double" "string" "boolean" "date" "thing" "entity" "attribute" "relation" "role" "rule"))
            (x-constants '("true" "false" "abstract" "id" "asc" "desc" "type"))
            (x-events '("offset" "limit" "group" "sort" "datatype" "key" "has" "plays" "relates"))
            (x-functions '("centrality" "using" "or" "not" "as" "via" "like" "when" "then" "regex"))
